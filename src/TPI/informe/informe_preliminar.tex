...
\begin{table}[htbp]
\centering
\caption{Comparación entre configuración FT tradicional y OTA obtenida por el Algoritmo Genético (15 ejecuciones independientes).}
...
La baja dispersión entre corridas ($\sigma = 0,01\,\text{kWh}$) valida la estabilidad estocástica del algoritmo y demuestra que la solución no depende de una inicialización favorable.
...
\begin{enumerate}
    ...
    \item Cuantificar el rédito energético de sistemas SAT y DAT sobre el mismo sitio, para contrastar la ganancia del OTA frente al costo adicional de un sistema con seguimiento.
\end{enumerate}
